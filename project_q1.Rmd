...
```{r}
for (i in 1:nrow(s_probs)){
  for (j in 1:ncol(s_probs)){
      value = unname(unlist(s_probs[i, j]))
      if (j == 7) {
         value = round(unname(unlist(s_probs[i, j])), 3)
         cat(value, " \\\\")
      }
      else cat(value, " & ")
  }
...
for (i in 1:8){
  for (j in 1:ncol(s_probs10)){
      value = unname(unlist(s_probs10[i, j]))
      if (j == 12) {
         value = round(unname(unlist(s_probs10[i, j])), 3)
         cat(value, " \\\\")
      }
      else cat(value, " & ")
  }
